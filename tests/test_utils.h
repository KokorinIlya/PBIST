...
template <typename T>
std::pair<std::unordered_set<T>, pasl::pctl::parray<T>> get_batch(
    uint64_t batch_size,
...
std::pair<std::unordered_set<T>, pasl::pctl::parray<T>> get_build_batch(
    ...
    std::default_random_engine& generator, std::uniform_int_distribution<T>& elements_distribution) 
{
    return get_batch(batch_size, generator, elements_distribution);
}
...
std::pair<pasl::pctl::parray<bool>, pasl::pctl::parray<T>> get_contains_batch(
    ...
    std::default_random_engine& generator, std::uniform_int_distribution<T>& elements_distribution) 
{
    pasl::pctl::parray<T> keys = get_batch(batch_size, generator, elements_distribution).second;
